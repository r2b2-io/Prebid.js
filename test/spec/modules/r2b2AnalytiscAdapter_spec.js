...
const {
  EVENTS: { NO_BID, AUCTION_INIT, BID_REQUESTED, BID_TIMEOUT, BID_RESPONSE, BID_REJECTED, BIDDER_DONE,
...
const R2B2_PID_1 = "r2b2.womanonly.cz/s2s/300x300/mobile";
const R2B2_PID_2 = "r2b2.womanonly.cz/s2s/320x150/mobile";
...
describe('r2b2 Analytics', function () {
  ...
  it('auction multiple init', (done) => {
    ...
    let auction_init_2 = deepClone(MOCK.AUCTION_INIT);
  ...
  it("bid rejected content", (done) => {
    let rejectedBid = deepClone(R2B2_AD_UNIT_2_BID);
    rejectedBid.rejectionReason = CONSTANTS.REJECTION_REASON.FLOOR_NOT_MET;
  ...
  it("empty auction", (done) => {
    let emptyAuctionInit = deepClone(MOCK.AUCTION_INIT);
    emptyAuctionInit.bidderRequests = [];
    let emptyAuctionEnd = deepClone(MOCK.AUCTION_END);
